...
def compare_files(test_file, reference_file):
    ...
    def normalize_precinct(precinct):
        """Normalize precinct name for comparison (e.g., 'Precinct 101' -> '101')."""
        # Remove common prefixes like "Precinct ", "Pct ", etc.
        precinct = precinct.strip()
        for prefix in ['Precinct ', 'Pct ', 'PCT ', 'precinct ']:
            if precinct.startswith(prefix):
                return precinct[len(prefix):]
        return precinct
    ...
    for (office, ref_precinct), ref_records in ref_data.items():
        ...
        if (office, ref_precinct) in test_data:
            ...
            # Try normalized precinct name match
            ref_norm = normalize_precinct(ref_precinct)
            for (test_office, test_prec), test_recs in test_data.items():
                test_norm = normalize_precinct(test_prec)
                if test_office == office and ref_norm == test_norm:
                    # Found normalized match - don't count as error since they're equivalent
        ...
        if test_records:
            ...
            for ref_row in ref_records['rows']:
                ...
                for t_row in test_records:
                    # Match if reference candidate is contained in test candidate (e.g., "Donald J. Trump" in "Donald J. Trump/JD Vance")
                    # Handle empty candidates for rows like Registered Voters
                    candidate_match = (
                        (not ref_row['candidate'] and not t_row['candidate']) or
                        (ref_row['candidate'] and t_row['candidate'] and ref_row['candidate'] in t_row['candidate'])
                    )
