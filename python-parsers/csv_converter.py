...
#            return False
    ...
    def is_office_header(self, line: str) -> bool:
        ...
        if any(line.startswith(x) for x in ['Registered Voters', 'Ballots Cast']):
            return False
        if line.startswith('US Representative') or line.startswith('State Treasurer') or line.startswith('Attorney General') or line.startswith('Auditor General') or line.startswith('President') or line.startswith('Secretary of State') or line.startswith("State Representative") or line.startswith("State Senator") or line.startswith("City of") or line.startswith("Constable") or line.startswith("Chief Justice") or line.startswith("Member, State") or line.startswith("Judge,"):
            return True
